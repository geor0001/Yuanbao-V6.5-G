# 专业增强版（全功能 / 完整特效与主题）—— 元宝 Bot Web 控制台 6.0Pro Max
# 保留全部能力：全场景液态玻璃（Liquid Glass）、各厂商主题（PHONE_EFFECTS）、
# 动态背景（视频背景 + 自定义图片）、🌌 鸿蒙空间光感（HarmonyOS 6 沉浸光效）、
# 🖼️ 在线图片编辑器（ImgEditor）、毛玻璃开关、16 套 QQ 主题、可视化音乐播放器、
# AI 聊天、@ 群成员（隐藏 ID）、个性化后端化与 config.json 可视化编辑器。
#!/usr/bin/env python3
"""
元宝 Bot Web 控制台 - 6.0Pro Max
"""
import sys
import os
import json
...
class EnhancedSpamSender:
    ...
    def __init__(self):
        ...
        # 撤回监听开关
        self.recall_monitor_enabled: bool = False
        # 撤回通知开关（是否发送撤回通知到群）
        self.recall_notify_enabled: bool = True
    ...
    async def _handle_recall_notification(self, push_json: dict):
        ...
        逻辑对照 group_monitor.py 的 _send_recall_notification：
        - 遍历所有 recall_list 项
        - 支持 msg_id 和 msg_seq 双路回退查找
        - 支持 image/file/sticker 三种媒体直接 protobuf 转发（不下载不重上传）
        - 代码块格式化（fence 反引号计数，防止原内容反引号提前关闭代码块）
        - 时间戳转换
        - 媒体发送成功后再发短通知文本；失败则回退纯文本通知（含代码块）
        """
        group_code = push_json.get("group_code", "")
                    ...
                    if mt == "image":
                        ...
                        if urls:
                            try:
                                ...
                                media_sent = True
                                print(f"[撤回] 已重发图片通知到群 {group_code}")
                            except Exception as e:
                                print(f"[撤回] 重发图片失败: {e}")
                        ...
                        if file_url:
                            try:
                                ...
                                await self.send_group_message(notif, target_group=notify_group)
                                media_sent = True
                                print(f"[撤回] 已重发文件通知到群 {group_code}")
                            except Exception as e:
                                print(f"[撤回] 重发文件失败: {e}")
                        ...
                        if sticker_id and package_id:
                            try:
                                ...
                                await self.send_group_message(notif, target_group=notify_group)
                                media_sent = True
                                print(f"[撤回] 已重发贴纸通知到群 {group_code}")
                            except Exception as e:
                                print(f"[撤回] 重发贴纸失败: {e}")

                if not media_sent and orig_content:
                    # 代码块格式化：计算 fence 长度，避免原内容反引号提前关闭
                    max_backticks = 0
                    bcount = 0
    ...
    async def send_image(self, image_path: str, group_code=None) -> bool:
        """发送本地图片到当前群（单张），可指定目标群"""
        return await self.send_images_multi([image_path], group_code=group_code)

    async def send_images_multi(self, image_paths: list, group_code=None) -> bool:
        """发送多张图片到目标群（一次消息包含多图）"""
        if not self.connected or not self.ws:
            return False
        ...
        try:
            msg = self._build_image_msg(images, group_code=group_code)
            await self.ws.send(msg)
...
settings = {
    ...
    'recall_monitor_enabled': app_config.get('RECALL_MONITOR_ENABLED', False),  # 撤回监听开关
    'recall_notify_enabled': app_config.get('RECALL_NOTIFY_ENABLED', True),  # 撤回通知开关
    'recall_notify_target': app_config.get('RECALL_NOTIFY_TARGET', 'original'),  # 撤回通知目标
}
...
sender.recall_monitor_enabled = settings.get('recall_monitor_enabled', False)
...
class PluginContext:
    ...
    def send_image(self, path, group_code=None):
        lp = self._loop()
        if lp and sender.connected:
            lp.create_task(sender.send_image(path, group_code=group_code))
...
def save_config():
    ...
    with _config_lock:
        ...
        config.update({
            ...
            'RECALL_MONITOR_ENABLED': getattr(sender, 'recall_monitor_enabled', False),
            'RECALL_NOTIFY_ENABLED': getattr(sender, 'recall_notify_enabled', True),
...
def api_send_image():
    ...
    try:
        if request.content_type and 'multipart' in request.content_type:
            ...
            try:
                ok = async_call(sender.send_image(
                    tmp.name,
                    group_code=request.form.get('group') or None,
                ))
            finally:
            ...
            ok = async_call(sender.send_image(
                path,
                group_code=data.get('group') or None,
            ))
            return (jsonify({'ok': True, 'message': '图片发送成功'}) if ok
                    else jsonify({'ok': False, 'message': '图片发送失败'}), 400)
    except Exception as e:
...
def api_get_settings():
    with settings_lock:
        # V4.3 支持 ?key=xxx 单项读取个性化偏好（存储在 settings['prefs'] 子字典）
        single = request.args.get('key')
        if single:
            prefs = settings.get('prefs', {}) or {}
            return jsonify({'ok': True, 'key': single, 'value': prefs.get(single)})
...
def api_update_settings():
    ...
    with settings_lock:
        ...
        if 'recall_notify_target' in data:
            tgt = data['recall_notify_target']
            if tgt in ('original', 'relay'):
                settings['recall_notify_target'] = sender.recall_notify_target = tgt

        # ── V4.3 个性化偏好（通用 key/value 持久化）：主题 / 效果 / 背景 / AI 配置 / 科技感 / 自定义主题 … ──
        if 'key' in data and 'value' in data:
            prefs = settings.setdefault('prefs', {})
            try:
                prefs[str(data['key'])] = data['value']
            except Exception as e:
                return jsonify({'ok': False, 'message': f'pref key 非法: {e}'}), 400
        elif isinstance(data.get('prefs'), dict):
            settings.setdefault('prefs', {}).update(data['prefs'])
...
# ── config.json 可视化编辑 API（V4.5 移植自 v5.0）──
CONFIG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config.json')
...
def api_save_config():
    ...
    try:
        ...
        if 'RECALL_NOTIFY_TARGET' in new_config:
            tgt = new_config['RECALL_NOTIFY_TARGET']
            if tgt in ('original', 'relay'):
                with settings_lock:
                    settings['recall_notify_target'] = sender.recall_notify_target = tgt
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("  元宝 Bot Web 控制台 - 6.0Pro Max")
    print("=" * 60)
